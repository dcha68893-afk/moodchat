...
// FIX (forensic audit 2026-06-21): friend:request / friend:accepted were emitted
// via raw io.to(room).emit(), which is fire-and-forget — if the receiver's socket
// isn't connected at that exact instant (Render free-tier cold start, brief
// reconnect gap, backgrounded tab), the event goes nowhere with no retry and no
// queue. wsService.sendToUser() already has offline-queue + HybridTransportRuntime
// fallback built in (used by messages/notifications) — use it here too so friend
// requests get the same delivery guarantees instead of silently vanishing.
...
router.post('/requests/send', apiRateLimiter, asyncHandler(async (req, res) => {
    try {
        ...
        // FIX: Use wsService.sendToUser() instead of raw io.to().emit() — this gives
        // friend requests the same offline-queue + HybridTransportRuntime fallback
        // that messages already have, so a receiver who is briefly disconnected
        // (cold start / reconnect gap) still gets the request once they're back,
        // instead of it silently vanishing.
        try {
            await wsService.sendToUser(receiverId, 'friend:request', requestPayload);
            await wsService.sendToUser(receiverId, 'FRIEND_REQUEST_RECEIVED', requestPayload);
        } catch (wsErr) {
            console.warn('[Friends /requests/send] wsService delivery failed, falling back to raw emit:', wsErr.message);
            const io = req.io || (req.app && req.app.get('io')) || global.__socketIO || null;
            if (io) {
...
router.post('/requests/:requestId/accept', apiRateLimiter, asyncHandler(async (req, res) => {
    try {
        ...
        // FIX: Use wsService.sendToUser() for the same offline-queue/HTR delivery
        // guarantees as the request-send path above, instead of raw fire-and-forget.
        let accepterProfile = { id: userId };
        let senderProfile2  = { id: friendRequest.requesterId };
        try {
            if (User) {
                ...
                if (senderUser) {
                    ...
                    senderProfile2 = { id: u.id, username: u.username||'', displayName: ([u.firstName,u.lastName].filter(Boolean).join(' ').trim())||u.username||'', avatar: u.avatar||null, status: u.status||'offline', lastSeen: u.lastSeen||null };
                }
            }
        ...
        const accepterPayload = {
            ...
            friendship:   { ...friendRequest.toJSON(), status: 'accepted' },
            user:         senderProfile2,
            friend:       senderProfile2,
            acceptedAt:   new Date().toISOString(),
        };
        try {
            await wsService.sendToUser(friendRequest.requesterId, 'friend:accepted', senderPayload);
            await wsService.sendToUser(userId, 'friend:accepted', accepterPayload);
        } catch (wsErr) {
            console.warn('[Friends /requests/:id/accept] wsService delivery failed, falling back to raw emit:', wsErr.message);
            const io = req.io || (req.app && req.app.get('io')) || global.__socketIO || null;
            if (io) {
