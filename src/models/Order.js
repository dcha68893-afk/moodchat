...
module.exports = (sequelize, DataTypes) => {
  const Order = sequelize.define(
    'Order',
    {
      ...
      status: { type: DataTypes.ENUM('pending', 'paid', 'shipped', 'delivered', 'cancelled', 'refunded'), defaultValue: 'pending', allowNull: false },
      quantity: { type: DataTypes.INTEGER, defaultValue: 1, validate: { min: 1 } },
      totalPrice: { type: DataTypes.DECIMAL(10, 2), allowNull: false, field: 'total_price', validate: { min: 0 } },
      ...
      deliveryAddress: { type: DataTypes.JSONB, defaultValue: {}, field: 'delivery_address' },
      trackingNumber: { type: DataTypes.STRING(255), allowNull: true, field: 'tracking_number' },
      notes: { type: DataTypes.TEXT, allowNull: true },
      metadata: { type: DataTypes.JSONB, defaultValue: {} },
      invoiceNumber: { type: DataTypes.STRING(40), allowNull: true, unique: true, field: 'invoice_number' },
      invoiceToken: { type: DataTypes.STRING(64), allowNull: true, unique: true, field: 'invoice_token' },
      invoiceIssuedAt: { type: DataTypes.DATE, allowNull: true, field: 'invoice_issued_at' },
  ...
  Order.associate = function (models) {
    ...
    if (models.Users) {
      Order.belongsTo(models.Users, { foreignKey: 'buyerId', as: 'buyer', constraints: false });
