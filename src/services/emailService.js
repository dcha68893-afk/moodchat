...
const FROM = process.env.EMAIL_FROM || 'Necpa <noreply@necpa.com>';
...
<div class="wrap">
<div class="hdr">Necpa Marketplace</div>
<div class="body"><h2 style="margin-top:0;color:#111">${title}</h2>${body}</div>
<div class="ft">Necpa · Nairobi, Kenya · You received this because you have an account with us.</div>
</div></body></html>`;
}
...
async function orderConfirmed(to, { orderId, items = [], total, currency = 'KES', deliveryAddress }) {
    ...
    const body = `
        ...
        <p>We'll notify you when your order ships. Thank you for shopping on Necpa!</p>`;
...
async function productApproved(to, { productTitle }) {
    const body = `
        ...
        <p>It is now live on Necpa Marketplace and visible to buyers.</p>`;
...
async function loginAttemptsAlert(to, { identifier, ip, attempts }) {
    const body = `
        <p>We detected <strong>${attempts}</strong> failed login attempts on your Necpa account.</p>
...
async function passwordResetEmail(to, { resetToken, resetUrl }) {
    ...
    const body = `
        <p>We received a request to reset your Necpa account password.</p>
    ...
    return send(to, 'Reset Your Necpa Password', _wrap('Password Reset Request 🔑', body));
}
...
async function verificationEmail(to, { verificationToken, verifyUrl }) {
    ...
    const body = `
        <p>Welcome to Necpa! Please verify your email address to activate your account.</p>
        <p><a class="btn" href="${link}">Verify Email</a></p>
        <p>This link will expire in 24 hours. If you didn't create a Necpa account, you can ignore this email.</p>`;
    return send(to, 'Verify Your Necpa Email', _wrap('Verify Your Email ✉️', body));
}

async function passwordChangedAlert(to) {
    const body = `
        <p>Your Necpa account password was just changed.</p>
    ...
    return send(to, 'Your Necpa Password Was Changed', _wrap('Password Changed 🔒', body));
}

async function newDeviceLoginAlert(to, { device, location, ip, time }) {
    const body = `
        <p>We noticed a new login to your Necpa account.</p>
    ...
    return send(to, 'New Login to Your Necpa Account', _wrap('New Device Login 📱', body));
}
