...
class CallService {
  ...
  async answerCall(callId, userId) {
    ...
    // Notify all participants that the call was answered
    try {
      const wsService = require('./webSocketService');
      const payload = { callId: call.id, answeredBy: parseInt(userId), status: 'in-progress', participants: call.participants };
      call.participants.forEach(pid => {
        wsService.sendToUser(pid, 'call:answered', payload);
        wsService.sendToUser(pid, 'call_answered', payload);
      });
    } catch (_wsErr) {}
  ...
  async rejectCall(callId, userId) {
    ...
    try {
      const wsService = require('./webSocketService');
      const payload = { callId: call.id, rejectedBy: parseInt(userId), status: call.status, participants: call.participants };
      call.participants.forEach(pid => {
        wsService.sendToUser(pid, 'call:rejected', payload);
        wsService.sendToUser(pid, 'call_rejected', payload);
      });
    } catch (_wsErr) {}
  ...
  async cancelCall(callId, userId) {
    ...
    // Notify all participants that caller cancelled
    try {
      const wsService = require('./webSocketService');
      const payload = { callId: call.id, cancelledBy: parseInt(userId), status: 'cancelled', participants: call.participants };
      call.participants.forEach(pid => {
        wsService.sendToUser(pid, 'call:cancelled', payload);
        wsService.sendToUser(pid, 'call_cancelled', payload);
        wsService.sendToUser(pid, 'call_force_ended', payload);
      });
    } catch (_wsErr) {}
  ...
  async endCall(callId, userId) {
    ...
    try {
      const wsService = require('./webSocketService');
      const payload = { callId: call.id, endedBy: parseInt(userId), status: call.status, duration, participants: call.participants };
      call.participants.forEach(pid => {
        wsService.sendToUser(pid, 'call:ended', payload);
        wsService.sendToUser(pid, 'call_ended', payload);
        wsService.sendToUser(pid, 'call_force_ended', payload);
      });
    } catch (_wsErr) {}
