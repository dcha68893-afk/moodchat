...
async function addMissingColumns() {
  ...
  const requiredColumns = {
    ...
    'Messages': [
      ...
      { name: 'disappearingTimer', type: Sequelize.INTEGER, allowNull: true },
      { name: 'isPinned', type: Sequelize.BOOLEAN, defaultValue: false, allowNull: false },
      { name: 'pinnedAt', type: Sequelize.DATE, allowNull: true },
      { name: 'pinnedBy', type: Sequelize.INTEGER, allowNull: true }
    ],
    'statuses': [
    ...
    'chat_participants': [
      { name: 'isMuted', type: Sequelize.BOOLEAN, defaultValue: false, allowNull: false },
      { name: 'mutedUntil', type: Sequelize.DATE, allowNull: true },
      { name: 'isPinned', type: Sequelize.BOOLEAN, defaultValue: false, allowNull: false },
      { name: 'pinnedAt', type: Sequelize.DATE, allowNull: true }
    ],
    'status_views': [
...
async function runFullMigration() {
  ...
  try {
    ...
    // STEP 7: Ensure messaging feature tables exist
    console.log('[Migration] STEP 7: Ensuring messaging feature tables exist...');
    try {
      await sequelize.query(`
        CREATE TABLE IF NOT EXISTS "starred_messages" (
          ...
          CONSTRAINT uq_starred_user_message UNIQUE ("userId", "messageId")
        );
        CREATE INDEX IF NOT EXISTS idx_starred_user    ON "starred_messages" ("userId");
        CREATE INDEX IF NOT EXISTS idx_starred_message ON "starred_messages" ("messageId");
        ...
        CREATE INDEX IF NOT EXISTS idx_sched_user   ON "scheduled_messages" ("userId");
        CREATE INDEX IF NOT EXISTS idx_sched_chat   ON "scheduled_messages" ("chatId");
        CREATE INDEX IF NOT EXISTS idx_sched_send   ON "scheduled_messages" ("sendAt");
        CREATE INDEX IF NOT EXISTS idx_sched_status ON "scheduled_messages" ("status","sendAt");
        ...
        CREATE TABLE IF NOT EXISTS "message_reports" (
          ...
          "createdAt"  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          "updatedAt"  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          CONSTRAINT uq_report_user_message UNIQUE ("reporterId", "messageId")
        );
        CREATE INDEX IF NOT EXISTS idx_report_message ON "message_reports" ("messageId");
        CREATE INDEX IF NOT EXISTS idx_report_status  ON "message_reports" ("status");
        ...
        CREATE TABLE IF NOT EXISTS "pinned_messages" (
          ...
          CONSTRAINT uq_pinned_chat_message UNIQUE ("chatId", "messageId")
        );
        CREATE INDEX IF NOT EXISTS idx_pinned_chat    ON "pinned_messages" ("chatId");
        CREATE INDEX IF NOT EXISTS idx_pinned_message ON "pinned_messages" ("messageId");
        ...
        CREATE INDEX IF NOT EXISTS idx_preview_url ON "link_previews" ("url");
      `);
      console.log('[Migration] ✅ Messaging feature tables (starred, scheduled, reports, pinned, link_previews) ensured');
    } catch (msgErr) {
      console.error('[Migration] ⚠️ Messaging feature table creation error (non-fatal):', msgErr.message);
    }

    // STEP 9: E2E Encryption tables
    console.log('[Migration] STEP 9: Ensuring E2E encryption tables...');
    try {
      await sequelize.query(`
        CREATE TABLE IF NOT EXISTS user_encryption_keys (
          ...
          "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          "updatedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          CONSTRAINT uq_user_key UNIQUE ("userId", "keyId")
        );
        CREATE INDEX IF NOT EXISTS idx_enc_keys_user   ON user_encryption_keys ("userId");
        CREATE INDEX IF NOT EXISTS idx_enc_keys_active ON user_encryption_keys ("userId","isActive");
        ...
        CREATE TABLE IF NOT EXISTS message_delivery_logs (
          ...
          "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
          CONSTRAINT uq_delivery_msg_user_event UNIQUE ("messageId","userId","event")
        );
        CREATE INDEX IF NOT EXISTS idx_delivery_msg  ON message_delivery_logs ("messageId");
        CREATE INDEX IF NOT EXISTS idx_delivery_user ON message_delivery_logs ("userId");
        CREATE INDEX IF NOT EXISTS idx_delivery_chat ON message_delivery_logs ("chatId");
        ...
        CREATE INDEX IF NOT EXISTS idx_edits_message ON message_edits ("messageId");
        ...
        CREATE TABLE IF NOT EXISTS message_views (
          ...
          CONSTRAINT uq_view_msg_user UNIQUE ("messageId","userId")
        );
        CREATE INDEX IF NOT EXISTS idx_views_message ON message_views ("messageId");
        ...
        CREATE INDEX IF NOT EXISTS idx_mentions_message ON message_mentions ("messageId");
        ...
        CREATE INDEX IF NOT EXISTS idx_totp_user ON user_totp_secrets ("userId");
      `);
      console.log('[Migration] ✅ E2E encryption + delivery + device + backup + push + totp tables ensured');
    } catch (msgErr) {
      console.error('[Migration] ⚠️ Messaging feature table creation error (non-fatal):', msgErr.message);
    }

    // STEP 8: Add GIN full-text search index on Messages.content
    console.log('[Migration] STEP 8: Adding full-text search index on Messages...');
    try {
      await sequelize.query(`
        DO $$
        BEGIN
          IF NOT EXISTS (
            ...
            UPDATE "Messages" SET "searchVector" = to_tsvector('english', COALESCE(content,''))
              WHERE "searchVector" IS NULL AND content IS NOT NULL;
          END IF;
        END$$;
      `);
      console.log('[Migration] ✅ Full-text search index on Messages ensured');
    } catch (ftsErr) {
      console.error('[Migration] ⚠️ FTS index error (non-fatal):', ftsErr.message);
    }
