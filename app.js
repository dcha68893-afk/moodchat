...
// FIX (STATUS-MEDIA-MIXED-CONTENT): Render terminates TLS at its proxy and
// forwards requests to this app over plain HTTP, setting X-Forwarded-Proto.
// Without `trust proxy`, Express's req.protocol always reports 'http' no
// matter what the client actually used — so every URL built from
// `req.protocol + '://' + req.get('host')` (status media uploads, file
// uploads, marketplace images, etc.) came out as http:// even though the
// frontend is served over https://. Browsers silently block that as mixed
// content, which is why uploaded status images/videos posted successfully
// but then rendered as "failed to load". This one setting fixes req.protocol
// (and req.secure) app-wide for every route that derives a URL from it.
